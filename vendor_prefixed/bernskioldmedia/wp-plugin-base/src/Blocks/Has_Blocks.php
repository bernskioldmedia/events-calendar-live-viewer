...
trait Has_Blocks
{
    ...
    public function register_blocks() : void
    {
        foreach ($this->blocks as $directory) {
            ...
            wp_register_script(static::get_block_prefix() . '-block-' . $name, static::get_url('dist/blocks/' . $name . '.js'), $asset_meta['dependencies'], $asset_meta['version'], \true);
            ...
            if (isset(static::$dynamic_blocks[$name])) {
                ...
                register_block_type($directory, ['render_callback' => [static::$dynamic_blocks[$name], 'render']]);
            } else {
